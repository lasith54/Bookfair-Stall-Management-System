...
async function seedAll() {
  try {
    ...
    // Seed Stalls - using specific layout matching InteractiveVenueMap
    console.log('Creating stalls (matching InteractiveVenueMap layout)...');
    const booksCategory = createdCategories.find(cat => cat.name === 'Books');
    const defaultCategoryId = booksCategory ? booksCategory._id : createdCategories[0]._id;
    const stallsData = generateStalls(defaultCategoryId);
    ...
    createdStalls.forEach(stall => {
      const zone = stall.location.zone;
    ...
    // Show zone breakdown
    const zones = ['L', 'A', 'B', 'C', 'R', 'M', 'X', 'BL', 'D', 'E', 'F', 'BR'];
    console.log('   - Zones:');
    for (const zone of zones) {
      const count = await Stall.countDocuments({ 'location.zone': zone });
      if (count > 0) {
        console.log(`     ${zone}: ${count} stalls`);
      }
    }
    console.log('');
